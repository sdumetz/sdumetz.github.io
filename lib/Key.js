...
export default class Key extends Element{
  ...
  toggle(){
    ...
    console.log("panel : ",parent.getElementsByClassName('key-panel'))
    for(let node of parent.getElementsByClassName('key-panel')){
      parent.removeChild(node); //Seems stupid but no better way
    }
    if(!this.active){
      if(this._el.nextSibling){
        parent.insertBefore(this.longtext,this._el.nextSibling);
      }else{
